...
func TestResolveFree(t *testing.T) {
	...
	for _,tt := range tests {
		for _, symbol := range tt.expectedSymbols {
			...
			if result != symbol {
				t.Errorf("expected %s tp resolve to %+v, got=%+v",symbol.Name,symbol,result)
			}
		}
		...
		for i, freeSymbol := range tt.expectedFreeSymbols {
			...
			if result != freeSymbol {
				t.Errorf("wrong free symbol. got=%+v, want=%+v",result,freeSymbol)
			}
		}
	}
}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, symbol := range expected {
		...
		if !ok {
			t.Errorf("name %s not resolable",symbol.Name)
			continue
		}
	...
	for _, s := range expectedUnresolvable {
		if _,ok := secondLocal.Resolve(s);ok {
			t.Errorf("name %s resolved, but was expected not to",s)
		}
